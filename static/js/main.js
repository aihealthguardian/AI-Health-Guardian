...
function processReportFile(file) {
  ...
  .then(data => {
      ...
      if (data.medicines && data.medicines.length) {
        ...
        data.medicines.forEach(m => {
          ...
          medicines.push({
            ...
            lastResetDate: null,
            takenToday: false,
            durationDays: m.duration_days ? parseInt(m.duration_days, 10) : 1,
            daysCompleted: 0,
    ...
    data.appointments.forEach(a => {
            if (!a.doctor && !a.hospital && !a.date) return;
      appointments.push({
        ...
        if (v.blood_pressure) {
          document.getElementById('bpVal').textContent = v.blood_pressure;
          const systolic = parseFloat(v.blood_pressure.split('/')[0]);
          if (!isNaN(systolic)) graphData.bp[lastIdx] = systolic;
        }
        if (v.sugar) {
          const sugarNum = v.sugar.replace(/\s*\(via Gemini\)/i, '').trim();
          document.getElementById('sugarVal').textContent = sugarNum;
          graphData.sugar[lastIdx] = Number(sugarNum);
        }
        if (v.weight) {
          const weightNum = v.weight.replace(/\s*\(via Gemini\)/i, '').trim();
          document.getElementById('weightVal').textContent = weightNum;
          graphData.weight[lastIdx] = Number(weightNum);
        }
        if (v.blood_group) {
          document.getElementById('bgVal').textContent = v.blood_group.replace(/\s*\(via Gemini\)/i, '').trim();
          const bgFieldGroup = document.getElementById('bgFieldGroup');
          if (bgFieldGroup) bgFieldGroup.style.display = 'none';
        }
        ...
        healthFormData.append("bp", v.blood_pressure || "");
        healthFormData.append("sugar", (v.sugar || "").replace(/\s*\(via Gemini\)/i, '').trim());
        healthFormData.append("weight", (v.weight || "").replace(/\s*\(via Gemini\)/i, '').trim());
               healthFormData.append("blood_group", (v.blood_group || "").replace(/\s*\(via Gemini\)/i, '').trim());
        if (data.report_date) {
          healthFormData.append("record_date", data.report_date);
        }
        ...
        .then(res => res.text())
        .then(() => loadHealthHistory())
        .catch(err => console.error('Auto health-save error:', err));

        showToast('✅ Health data updated from report', 'green');
      }
...
async function loadHealthHistory() {
  try {
    ...
    // Blood group is a one-time value, not tied to monthly bp/sugar/weight
    // history — check it before the months-based early return below, so it
    // still hides correctly even when there's no monthly data yet.
    if (data.blood_group && data.blood_group.length) {
      let savedBloodGroup = null;
      data.blood_group.forEach(bg => { if (bg) savedBloodGroup = bg; });
      if (savedBloodGroup) {
        document.getElementById('bgVal').textContent = savedBloodGroup;
        const bgFieldGroup = document.getElementById('bgFieldGroup');
        if (bgFieldGroup) bgFieldGroup.style.display = 'none';
      }
    }
    ...
    data.months.forEach((raw, idx) => {
      ...
      if (key) {
        map[key] = {
          ...
          bp_raw: data.bp_raw ? data.bp_raw[idx] : null,
          blood_group: data.blood_group ? data.blood_group[idx] : null
        };
      }
    });
    ...
    if (lastKey) {
      ...
      if (entry.bp_raw) document.getElementById('bpVal').textContent = entry.bp_raw;
      updateHealthScoreDisplay();
    }
  } catch (e) {
...
function saveHealthData() {
  ...
  if (bg) {
    document.getElementById('bgVal').textContent = bg;
    const bgFieldGroup = document.getElementById('bgFieldGroup');
    if (bgFieldGroup) bgFieldGroup.style.display = 'none';
  }
...
function renderHomeMedicineList() {
  ...
  container.innerHTML = medicines.map(m => {
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding-bottom:10px;border-bottom:1px solid var(--border);${active ? '' : 'opacity:0.55;'}">
      ...
      ${(!m.takenToday && !m.missedToday && isMedicineCourseActive(m)) ? `
